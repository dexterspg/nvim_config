...
function M.map_java_keys(opts)
    ...
    local spring_boot_run = 'mvn sprig-boot:run -Dspring-boot.run.profiles=local'
    local command = ':lua require("togleterm").exec("' .. spring_boot_run .. '")<CR>'
    vim.keymap.set('n', '<leader>sb', command)
